...
def main():
    ...
    BUCKET_NAME = "soumilshah-hudi-demos"
    ...
    redshift_options = {
        'url': 'jdbc:redshift://<domain>:5439/<DATABASENAME>',
        'dbtable': "landing.orders",
        'user': '<USER NAME>',
        'password': '<PASSWORD>',
        'redshiftTmpDir': "s3://<BUCKET>/tmp/",
    }
    spark_df.write \
        .format('jdbc') \
        .option('url', redshift_options['url']) \
        .option('dbtable', redshift_options['dbtable']) \
        .option('user', redshift_options['user']) \
        .option('password', redshift_options['password']) \
        .option('tempdir', redshift_options['redshiftTmpDir']) \
        .mode('append') \
        .save()
